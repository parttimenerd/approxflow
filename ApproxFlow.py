...
def decide_info_flow(klass, classpath, cnf_only, unwind_limit=32, int_sz=-1, cbmc_timeout=None, modelcounter_timeout=None):
  ...
  args = [klass + ".class", "--classpath",
          os.path.abspath(os.path.dirname(__file__)) + "/jbmc-core-models.jar:.", "--dimacs"]
...
def build_class(klass, classpath):
  subprocess.check_call("cd {}; javac {}.java".format(classpath, klass), shell=True)
...
def main(argv):
  ...
  decide_info_flow(klass, classpath, cnf_only,
                   unwind_limit=int(os.getenv("UNWIND", "32")), cbmc_timeout=1000, modelcounter_timeout=1000)
